...
var app = angular.module("app.angular.utilities", [ "app.angular.debug"]);
...
app.factory('languageFactory', [ "$http", function($http) {
	...
	return {
		readJson : function(callback) {
			return $http.get('./app-language/en-US.json').then(function(data) {
				console.log(data);
				return data;
			});
...
.factory('formTools', ["logger", function (logger) {
        return {
            extractFormControllers: function (element) {
                logger.log("Searching for one level nested forms, Start");
                var formList = element.find('form');
                var controllerList = [];
                if (formList.length) {
                    logger.log("Size of forms array found:" + formList.length);
                }
                for (var i = 0; i < formList.length; i++) {
                    var form = formList.eq(i);
                    var formController = form.controller('form');
                    logger.log(form + "Form No :" + i);
                    controllerList.push(formController);
                }
                logger.log(controllerList);
                logger.log("Searching for one level nested forms, End");
                return controllerList;
            }
        };
}])
.factory("objectEngine", [function () {
        return {
            ...
            /**
             * This is a comparator function which is used to compare the values for the "filter" of "select"
             * because the native angular filter is not sufficient
             * @param field1: this is the first value to compare
             * @param field2: this is the second value to compare
             * @returns {boolean}
             */
            exactStringComparator: function (field1, field2) {
                if (!field1 || !field2) {
                    return false;
                }
            ...
            dateEquals: function (date1, date2) {
                if (date1 instanceof Date) {
                    var baseYear = date1.getFullYear();
                    var baseMonth = date1.getMonth();
                    var baseDay = date1.getDate();
                    if (date2 && date2 instanceof Date) {
                        var checkYear = date2.getFullYear();
                        var checkMonth = date2.getMonth();
                        var checkDay = date2.getDate();

                        if (baseYear === checkYear && baseMonth === checkMonth && baseDay === checkDay) {
                            return true;
                        }
                    }
                }
    ...
    .factory("getChartOptions", [function () {
    	return {
    		...
    		getcumulativeLineChartOptions : function(){
	        	return {
	                chart: {
   ...
   .factory('AppWebServices', [ "$http", 'contextFactory', '$rootScope', function($http, contextFactory, $rootScope) {
	
	var returnedObject = {
            loadData: function (url) {
            	var token = $rootScope.authToken;
            	var req = contextFactory.getDataServiceRequest(url, token);
            	$http(req).success(function (response, status, headers, config) {
    	        	console.log(response);
    	        	if (response.error.errorCode == 0 && response.error.errorDescription == '') {
    ...
    .factory("graphUtil", ['contextFactory', 'auth_token', '$rootScope', '$http', 'getChartOptions', 'AppWebServices', function (contextFactory, auth_token, $rootScope, $http, getChartOptions, AppWebServices) {
    	/*
    	 *  var olddata = {"fruit":{"apples":10,"pears":21}};
			var newdata = {};
			newdata['vegetables'] = {"carrots": 2, "potatoes" : 5};
			$.extend(true, olddata, newdata);
			This will generate: {"fruit":{"apples":10,"pears":21}, "vegetables":{"carrots":2,"potatoes":5}};
    	 */
    	return {
    		lineChart: function (url) {
    			
    			var data = {};
		        var result = {};
		        var options = {};
		        
    			//var req = contextFactory.getDataServiceRequest('/dashboardservice/lineChart', token);
    			var getData = AppWebServices.loadData('/dashboardservice/lineChart' + "?isLightWeight=n");
    			console.log("****************************");
    			console.log(getData);
    	      
    	        data = sinAndCos();
				options = getChartOptions.getlineChartOptions();
            ...
            cumulativeLineChart: function(url){
		        var result = {};
		        var options = {};
		        var data = [];
		        
    			var req = contextFactory.getDataServiceRequest('/dashboardservice/cumulativeLineChart', token);
    	        $http(req).success(function (response, status, headers, config) {
    	        	if (response.error.errorCode == 0 && response.error.errorDescription == '') {
						console.log("DRAW THE CUMULATIVE CHART");
    	        ...
    	        //data = getData();
				options = getChartOptions.getcumulativeLineChartOptions();
				
    	        result = {"options":options, "data":data};
    	        
            	return result;
            },
            stackedAreaChart: function(url){
            	return "Test stackedAreaChart";
            },
            multiBarChart: function(url){
            	return "Test multiBarChart";
            },
            discreteBarChart: function(url){
            	return "Test discreteBarChart";
            },
            historicalBarChart: function(url){
            	return "Test historicalBarChart";
            },
            multiBarHorizontalChart: function(url){
            	return "Test multiBarHorizontalChart";
            },
            pieChart: function(url){
            	return "Test pieChart";
            },
            scatterChart: function(url){
            	return "Test scatterChart";
            },
            lineWithFocusChart: function(url){
            	return "Test lineWithFocusChart";
            },
            scatterPlusLineChart: function(url){
            	return "Test scatterPlusLineChart";
            },
            linePlusBarWithFocusChart: function(url){
            	return "Test linePlusBarWithFocusChart";
            },
            donutChart: function(url){
            	return "Test donutChart";
            },
            bulletChart: function(url){
            	return "Test bulletChart";
            },
            sparklinePlus: function(url){
            	return "Test sparklinePlus";
            },
            parallelCoordinates: function(url){
            	return "Test parallelCoordinates";
            },
            multiChart: function(url){
            	return "Test multiChart";
            },
            candlestickBarChart: function(url){
            	return "Test candlestickBarChart";
            },
            sunburstChart: function(url){
            	return "Test sunburstChart";
            },
            ohlcBarChart: function(url){
            	return "Test ohlcBarChart";
            },
            boxPlotChart: function(url){
            	return "Test boxPlotChart";
            },
            forceDirectedGraph: function(url){
            	return "Test forceDirectedGraph";
            }
    	};
    }]);
